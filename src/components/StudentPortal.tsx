...
export default function StudentPortal({
  ...
  const enrollments = applications
    ...
    .map((app, idx) => {
      ...
      const appId = app.$id || app.id || `app_${idx}`;
      return {
        id: appId,
        courseId: progDetail?.id || appId,
        title: app.program || 'Formation',
        instructor: 'Jury académique IDLA',
        category: progDetail?.category || 'Formation',
        level: 'Tous niveaux' as any,
        duration: progDetail?.duration || '12 mois',
        progressPercent: 0,
        totalLessons: 10,
        completedLessons: 0,
        nextLessonTitle: 'Module 1 : Introduction & Fondamentaux',
        status: 'en cours' as const,
        enrolledAt: app.dateApplied || app.$createdAt || new Date().toISOString(),
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {applications.slice(0, 3).map((app, idx) => {
                  ...
                  const appId = app.$id || app.id || `app_${idx}`;
                  const createdDate = app.dateApplied || app.$createdAt;
                  const formattedDate = createdDate ? new Date(createdDate).toLocaleDateString('fr-FR') : 'Récemment';
                  return (
                    <div key={appId} className="bg-bg-secondary border border-border-primary rounded-2xl p-5 shadow-sm space-y-3">
          ...
          {progSection === 'mes-candidatures' && (
            <div className="space-y-4">
              {appsLoading ? (
                ...
                (() => {
                  const app = applications.find((a) => (a.$id || a.id) === selectedAppId);
                  if (!app) {
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-bg-secondary border border-border-primary rounded-2xl p-5 shadow-sm">
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-text-secondary">Dossier n° #{appId.slice(-6).toUpperCase()}</p>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {applications.map((app, idx) => {
                    ...
                    const appId = app.$id || app.id || `app_${idx}`;
                    const createdDate = app.dateApplied || app.$createdAt;
                    const formattedDate = createdDate ? new Date(createdDate).toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' }) : 'Récemment';
                    const step = STEP[app.status] ?? 1;
                    const steps = [
                      { n: 1, label: 'Déposé' },
                      { n: 2, label: 'En examen' },
                    ...
                    return (
                      <div key={appId} className="bg-bg-secondary border border-border-primary rounded-2xl p-6 shadow-sm space-y-5 flex flex-col justify-between">
                        <div className="space-y-5">
                          ...
                          <div className="grid grid-cols-2 gap-3 text-xs border-t border-border-primary/40 pt-4">
                            ...
                            <div>
                              ...
                              <p className="text-text-primary font-bold font-mono mt-0.5">#{appId.slice(-6).toUpperCase()}</p>
                            </div>
                          </div>
                        </div>
