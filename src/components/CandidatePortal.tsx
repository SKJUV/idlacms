...
export default function CandidatePortal({ onBackToHome, onLoginSuccess, isLoggedIn, knownEmail }: CandidatePortalProps) {
  // Login Form States
  const [email, setEmail] = useState(knownEmail || 'jean.dupont@email.com');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Tenter de se connecter via Appwrite
      await account.createEmailPasswordSession(email, password);
      onLoginSuccess();
    } catch (err: any) {
      console.warn("Connexion Appwrite échouée ou non configurée, repli vers les données simulées (mock).", err);
      // Mode démo (mock) si Appwrite n'est pas configuré ou en cas de problème de credentials locaux
      if (email === 'jean.dupont@email.com' && password === 'password123') {
        onLoginSuccess();
      } else {
        setLoginError(err.message || 'Identifiants incorrects. Utilisez jean.dupont@email.com / password123');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  // Données dérivées du dossier réel (avec repli démo si aucune candidature trouvée)
  const displayName = application?.name || 'Jean Dupont';
  const displayProgram = application?.program || 'Executive MBA Stratégie Digitale';
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {!application && !isLoadingApplication && (
                <p className="text-xs text-amber-600 font-semibold mt-1">Aucune candidature trouvée pour {email} — aperçu de démonstration.</p>
              )}
            </div>
          </div>
          {isAccepted ? (
