...
export default function PublicPortal({ activeTab, setActiveTab, onApplyNow, programs, news, testimonials, onSubmitTestimonial, onSubmitDonation }: PublicPortalProps) {
  ...
  if (activeTab === 'actualites') {
    return (
      <div className="bg-bg-primary text-text-primary min-h-screen py-10 px-4 sm:px-6">
        <div className="max-w-2xl mx-auto space-y-6">
          {/* Header Fil d'Actualités Facebook Style */}
          <div className="bg-bg-secondary p-5 rounded-2xl border border-border-primary shadow-sm space-y-4 text-center">
            <div className="flex items-center justify-center gap-2">
              <div className="w-10 h-10 rounded-full bg-brand-primary/10 text-brand-primary flex items-center justify-center font-black text-sm border border-brand-primary/20">
                IDLA
              </div>
              <div className="text-left">
                <h1 className="font-sans font-bold text-xl text-text-primary leading-none">Fil d'Actualités Académiques</h1>
                <p className="text-xs text-text-secondary mt-1">Publications officielles, annonces et événements en direct</p>
              </div>
            </div>

            {/* Filter pills horizontal scroll */}
            <div className="flex items-center gap-2 overflow-x-auto pb-1 pt-2 no-scrollbar justify-center sm:justify-start">
                  ...
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-full text-xs font-bold transition-all shrink-0 cursor-pointer flex items-center gap-1.5 border ${
                      isSelected
                        ...
                        : 'bg-bg-primary text-text-secondary hover:text-text-primary border-border-primary'
                    }`}
                  >
                    <span>{cat === 'Tous' ? 'Tout le fil' : cat}</span>
                    <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${isSelected ? 'bg-white/20 text-white' : 'bg-border-primary/50 text-text-secondary'}`}>
                      {count}
                    </span>
                  </button>
                );
          ...
          {/* Posts Feed Single-Column (à la suite comme Facebook) */}
          {filteredNews.length > 0 ? (
            <div className="space-y-6">
              {filteredNews.map((n) => (
                <article
                  key={n.id}
                  className="bg-bg-secondary border border-border-primary rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all space-y-3 p-5"
                >
                  {/* Header Post Facebook-style */}
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-bold text-brand-primary bg-brand-primary/10 border border-brand-primary/20 px-2 py-0.5 rounded-full">
                            {n.category}
                          </span>
                        </div>
                    ...
                    {n.isFeatured && (
                      <span className="bg-amber-500/10 text-amber-700 text-[10px] font-bold px-2.5 py-1 rounded-full border border-amber-500/20 flex items-center gap-1">
                        <Flame className="w-3 h-3 text-amber-600" /> Épinglé
                      </span>
                    )}
                  </div>
                  ...
                  {/* Post Action Footer Facebook Style (J'aime / Partager / Voir l'article) */}
            ...
            <div className="bg-bg-secondary p-12 text-center rounded-2xl border border-border-primary space-y-2">
              <p className="text-text-secondary font-semibold text-sm">Aucune publication dans cette catégorie pour le moment.</p>
              <button
