...
export default function PublicPortal({ activeTab, setActiveTab, onApplyNow, programs, news, testimonials, onSubmitTestimonial, onSubmitDonation }: PublicPortalProps) {
  ...
  if (activeTab === 'programmes') {
    return (
      <div className="bg-bg-primary text-text-primary min-h-screen py-10 px-6 md:px-12 relative">
        <div className="max-w-[1440px] mx-auto space-y-8">
          ...
          {/* Sidebar & Content Layout (Style Hugging Face, Sticky & Fixed au Scroll) */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            {/* Sidebar Left (Col 3) - Sticky Position Fixe */}
            <div className="lg:col-span-3 shrink-0 lg:sticky lg:top-24 lg:max-h-[calc(100vh-7rem)] lg:overflow-y-auto pr-1">
              <ProgramFilterBar
                filters={filters}
                onFilterChange={setFilters}
                onReset={() => setFilters(INITIAL_FILTER_STATE)}
        ...
        {/* Bouton Flottant pour remonter vers le haut au scroll (Scroll-To-Top) */}
        {showScrollTop && (
          <button
            onClick={scrollToTop}
            title="Remonter en haut de la page"
            className="fixed bottom-6 right-6 z-50 bg-[#006c49] hover:bg-slate-800 text-white p-3.5 rounded-full shadow-2xl transition-all duration-300 hover:scale-110 flex items-center justify-center cursor-pointer border border-white/20"
          >
